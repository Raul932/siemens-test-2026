...
* I treated `Beverage` and `Extra` as master data/catalogs. Therefore, the `OrderItem` has a simple directed association to them, rather than a composition or aggregation. An order simply references an item from the menu; it doesn't "own" the definition of a Latte.
